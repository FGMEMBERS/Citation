...
var add_waypoint = func (ID, name, type, coord) { #add a waypoint to a route
    ...
    if (name == nil) { waypoint.getNode("name",1).setValue(""); }
    else             { waypoint.getNode("name",1).setValue(name); }
    waypoint.getNode("desc",1).setValue("no info");
...
var Waypoint_to_scratch = func (node) {
    ...
    scratch.getNode("ident",1).setValue(node.getNode("ID").getValue());
